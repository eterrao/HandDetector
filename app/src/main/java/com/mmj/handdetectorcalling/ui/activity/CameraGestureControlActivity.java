...
import com.mmj.handdetectorcalling.ui.view.CustomSurfaceView;
import com.mmj.handdetectorcalling.utils.CustomCameraUtils;
import com.mmj.handdetectorcalling.utils.SystUtils;
import com.mmj.handdetectorcalling.utils.imageProcessing.ColorBlobDetector;
 ...
 */
public class CameraGestureControlActivity extends BaseActivity implements View.OnTouchListener, CameraBridgeViewBase.CvCameraViewListener2 {
    // 加载JNI的依赖包: 名为libopencv_java3.so
    static {
        System.loadLibrary("opencv_java3");
    }

    private static final String TAG = CameraGestureControlActivity.class.getSimpleName();

    private static final int DETECT_TYPE = 2; // 识别两根手指
        ...
        public void onManagerConnected(int status) {
            switch (status) {
                case LoaderCallbackInterface.SUCCESS: {
                    ...
                    mOpenCvCameraView.setOnTouchListener(CameraGestureControlActivity.this);
                    // 640x480
                }
                break;
                default: {
    ...
    protected void initData() {
        initDebuger();
        if (!TextUtils.isEmpty(SystUtils.capturePath)) {
            capturePreviewIV.setImageBitmap(BitmapFactory.decodeFile(SystUtils.capturePath));
        }
    }
    ...
    private void initWidget() {
        ...
        minTresholdSeekbarText = (TextView) findViewById(R.id.tv_seek_to);
        numberOfFingersText = (TextView) findViewById(R.id.numberOfFingers);
        minTresholdSeekbar = (SeekBar) findViewById(R.id.seekBar1);
        capturePreviewIV = (ImageView) findViewById(R.id.iv_capture_preview);
    }
    ...
    public void onCameraViewStarted(int width, int height) {
        ...
        */
        ...
        SystUtils.showToast("Focus mode : " + cParams.getFocusMode());
    ...
    private synchronized void takePhoto() {
        SystUtils.cameraCanCaptured = false;
        if (mOpenCvCameraView != null) {
            SystUtils.o("mOpenCvCameraView.getCamera()=> " + (mOpenCvCameraView.getCamera() == null ? true : false));
            mOpenCvCameraView.getCamera().takePicture(null, null, pictureCallback);
        }
    }
    ...
    private Camera.PictureCallback pictureCallback = new Camera.PictureCallback() {
        ...
        public void onPictureTaken(byte[] data, Camera camera) {
            SystUtils.o("data===>  cameraCanCaptured=> " + SystUtils.cameraCanCaptured);
            if (data != null && data.length > 0) {
                Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);
                SystUtils.capturePath = CustomCameraUtils.saveBitmap2FileUseQuality(bitmap, 100);
                SystUtils.o("capturePath=> " + SystUtils.capturePath);
                capturePreviewIV.setImageBitmap(BitmapFactory.decodeFile(SystUtils.capturePath));
            }
            SystUtils.cameraCanCaptured = true;
